...
/**
 * 浮动view
 * Created by xiaoyu.zhang on 2016/12/15 17:41
...
public class FloatView extends LinearLayout {
    ...
    public FloatView(Context context, int x, int y, int layoutres, int flags) {
        super(context);
        View view = LayoutInflater.from(getContext()).inflate(layoutres, null);
        init(view, x, y, flags);
    }

    public FloatView(Context context, int x, int y, View childView, int flags) {
        super(context);
        init(childView, x, y, flags);
    }

    private void init(View childView, int x, int y) {
        init(childView, x, y, WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE);
    }

    private void init(View childView, int x, int y, int flags) {
