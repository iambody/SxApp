...
public abstract class BaseAdapter<MODEL extends BaseModel, LISTENER extends OnBaseListener, HOLDER extends RecyclerView.ViewHolder>
    ...
    protected RecyclerView.ViewHolder onCreateErrorViewHolder(ViewGroup parent) {
        return new ErrorHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_no_data, null));
    }

    protected void bindErrorHolder(MODEL model, HOLDER holder) {
        if (holder instanceof ErrorHolder) {
            ...
            if (!model.isError) {
                eh.errorDataView.setData(model.noDataIvSize, model.noDataIvResId, model.noDataTvStr, model.noDataBtnStr, model.noDataBtnWidth, model.noDataBtnHeight);
            } else {
                eh.errorDataView.setData(model.errorStatus);
            }

            eh.errorDataView.setOnErrorDataListener(() -> listener.onErrorClickListener());
